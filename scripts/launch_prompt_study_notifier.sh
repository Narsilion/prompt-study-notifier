...
notify() {
  /usr/bin/osascript -e "display notification \"$1\" with title \"Prompt Study Notifier\"" >/dev/null 2>&1 || true
}
...
if [[ ! -x ".venv/bin/prompt-study-notifier" ]]; then
...
log "Starting Prompt Study Notifier server"
notify "Starting local server..."
nohup env PYTHONPATH="${PROJECT_DIR}" .venv/bin/python -m prompt_study_notifier.main > "${SERVER_LOG}" 2>&1 &
